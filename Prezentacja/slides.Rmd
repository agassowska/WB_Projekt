...
1. <span style = "color:red;"> **kroku E** </span>, dla aktualnego układu parametrów rozkładu wyestymuj prawdopodobieństwa przynależenia do grup/wartości, które zmaksymalizują wartość funkcji wiarygodności,  
...
## Wizualizacja danych imputowanych (DO WSTAWIENIA)
...
```{r summary_wine}
summary(aggr(df, sortVar = TRUE))$combinations


```
## Metoda FAMD

<font size = "4"> Metoda FAMD (factor analysis for mixed data) to "połączenie" dwóch omawianych wcześniej metod PCA i MCA. Z grubsza można by powiedzieć, że FAMD działa jak PCA dla danych ciągłych i jak MCA dla danych kategorycznych. Zmienne kategoryczne są dyskretyzowane, aby miała sens macierz relacji dla wszystkich zmiennych. 
</font>

## Estymacja ncp dla FAMD

```{r ncp5, dev.args=list(bg='transparent'), fig.height=3.3, fig.align='center', cache=TRUE}  
#nb5 <- estim_ncpFAMD(wine, method='Regularized', 
                    #verbose=FALSE, method.cv="Kfold")
#plot(1:5, nb1$criterion, xlab="nb dim", ylab="MSEP")
#title(paste("Result = ", nb4$ncp))
```  
...
```{r imputation5}
res.comp <- imputeFAMD(df, ncp=2, 
                      method="Regularized") 
# Dane po imputacji
res.comp$tab.disj[1:7,c(1:6,10)]
```
...
```{r uncert4, dev.args=list(bg='transparent'), fig.align="center", fig.show="hold", cache=T}
...
```{r}
res.comp <- imputeMFA(df, group = c(2,6), type=c("n","s"))
# Dane po imputacji
...
https://www.youtube.com/watch?v=FgakZw6K1QQ&t=710s