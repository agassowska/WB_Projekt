...
```{r summary_wine, dev.args=list(bg='transparent'), fig.align='center', echo=F, results='hide'}
...
## Imputacja
```{r imputation7, eval=F}
res.comp$completeObs[1:7, ]
```

## GRAFY DLA MFA

```{r, dev.args=list(bg='transparent'), fig.height=4, fig.align="center", eval=F}
res.mfa <- MFA(df, 
               group = c(2,2,4),
               type = c("n", "s", "s"),
               graph = TRUE, ncp=2)
