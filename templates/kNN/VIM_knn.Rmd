```{r kNN_VIM_knn_{{id}}, include=FALSE, warning=FALSE, message=FALSE}
...
result <- train_and_test(train=train, test=test, learner=glearner, target=target, title=paste0('kNN + VIM (knn) + ', name))

test_data[nrow(test_data)+1, ] <- c('kNN', 'VIM_knn', name, result$test_auc, result$test_bacc, result$test_mcc)
```

### `r name` {.tabset .tabset-fade .tabset-pills}

#### Crossvalidation results
```{r kNN_VIM_knn_{{id}}_crossvalidation, echo=FALSE, fig.align='center', fig.width=10, fig.height=5, out.width='100%', warning=FALSE, message=FALSE}
result$cv_plot
```

#### Imputation times
```{r kNN_VIM_knn_{{id}}_times, echo=FALSE}
times <- read.csv('./time_data/VIM_knn/time_data.csv')
cat(paste('Imputation time: ', round(times[times[['dataset']]==name, 'imputation_time'], 3)))
```

#### Test set results
```{r kNN_VIM_knn_{{id}}_measures, echo=FALSE}
...
#### Missings overview
```{r kNN_VIM_knn_{{id}}_overview, echo=FALSE, fig.align='center', fig.width=8, fig.height=6, warning=FALSE, message=FALSE}
