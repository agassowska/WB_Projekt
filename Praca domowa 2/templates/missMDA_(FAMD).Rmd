```{r missdMDA_(FAMD)_{{id}}, include=FALSE}
load_result <- load({{id}})
...
```{r missMDA_(FAMD)_{{id}}_crossvalidation, echo=FALSE, fig.align='center', fig.width=10, fig.height=5, out.width='100%'}
result$cv_plot
```

#### Imputation times
```{r missMDA_(FAMD)_{{id}}_times, echo=FALSE}
...
#### Test set results
```{r missMDA_(FAMD)_{{id}}_measures, echo=FALSE}
...
#### Dataset overview
```{r missMDA_(FAMD)_{{id}}_overview, echo=FALSE, fig.align='center', fig.width=8, fig.height=6}
vis_dat(dataset)
```